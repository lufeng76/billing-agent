...
def return_instructions_bigquery() -> str:
    ...
    instruction_prompt_bqml_v1 = f"""
      ...
      2. Then, use expand_to_actual_billing_tables tool to expand initial SQL to customer's actual billing tables, to get the final SQL statement.
      2. You should also validate the final SQL you have created for syntax and function errors (Use run_bigquery_validation tool). If there are any errors, you should go back and address the error in the SQL. Recreate the SQL based by addressing the error.
      ...
      NOTE: you should ALWAYS USE THE TOOLS ({db_tool_name}, expand_to_actual_billing_tables AND run_bigquery_validation) to generate SQL, not make up SQL WITHOUT CALLING TOOLS.
